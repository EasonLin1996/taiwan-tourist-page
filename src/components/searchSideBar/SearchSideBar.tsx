...
export const SearchSideBar: React.FC<PropsType> = (props) => {
  const { handleAreaChange } = props;
  return (
    <>
      <List
        ...
        renderItem={item => (
          <List.Item>
            <Link to={`/searchpage?category=${item.key}`} className={styles.link}>{item.name}</Link>
          </List.Item>
        )}
      />
